...
uint8_t CRYPTO_NONCE[8] = {0xde, 0xad, 0xbe, 0xef, 0xc0, 0xfe, 0xba, 0xb1};
...
static void chacha20_init_block(chacha20_context_t *ctx, uint8_t key[], uint8_t nonce[])
{
...
static void chacha20_block_set_counter(chacha20_context_t *ctx, uint64_t counter)
{
	ctx->state[12] = (uint32_t)counter;
	ctx->state[13] = pack4(ctx->nonce + 0 * 4) + (uint32_t)(counter >> 32);
}

static void chacha20_block_next(chacha20_context_t *ctx) {
...
void chacha20_init_context(chacha20_context_t *ctx, uint8_t key[], uint8_t nonce[], uint64_t counter)
{
	memset(ctx, 0, sizeof(chacha20_context_t));
...
void chacha20_xor(chacha20_context_t *ctx, uint8_t *bytes, size_t n_bytes)
{
...
int main(int argc, char ** argv) {
    ...
    chacha20_context_t * ctx = malloc(sizeof(chacha20_context_t));
    if (!ctx) {
        printf("alloc failed");
        return 1;
    }

    uint8_t key[32] = {72, 58, 60, 234, 33, 219, 24, 146, 59, 107, 245, 181, 220, 113, 36, 62, 197, 74, 68, 189, 9, 149, 65, 82, 96, 154, 206, 29, 72, 48, 145, 216};
    chacha20_init_context(ctx, key, CRYPTO_NONCE, 0xfacecc43);

    uint8_t plaintext[32] = {0};
    memcpy(plaintext, argv[1], 29);

    chacha20_xor(ctx, plaintext, 29);
    chacha20_xor(ctx, plaintext, 29);
    chacha20_xor(ctx, plaintext, 29);
